...
class Hopfield:  # Создание класса Hopfield для работы с сетью Хопфилда
    ...
    Hopfield(images, test_image, view="short")
        ...
        и синхронным режимим. Сразу же производит все вычисления. В экземпляр
        передавать обычные одномерные массивы заданыые через квадратные скобки
        ...
        print_zeroed_w() - Вывести матрицу "W" с нулями на гловной диагонали (занулённую) (Шаг 4)
    ...
    def __init__(self, images, test_image, view="short"):  # Функция инициализации экземляра объекта класса
        ...
        self.xt_multiply_x = []  # Инициализация массива хранящего значения произведения матр. "X" трансп. на матр. "X"
        self.w = []  # Инициализация массива хранящего матрицу "W" (матрицу весовых коэффициентов)
        self.zeroed_w = []  # Инициализация массива хранящего занулённую матрицу "W" (матрицу весовых коэффициентов)
        self.y_vector = []  # Инициализация массива хранящего вектор "y" (тестовый образ)
        self.w_multiply_y = []  # Инициализация массива хранящего произведение занулённой матрицы "W" на вектор "y"
        self.tanh_w_multiply_y = []  # Ин. мас. храниящего значения ф-и "tanh" от пр. зан. матр. "W" на век. "y"
        ...
        self.calculate_zeroed_w_multiply_y()  # Вызов функции вычисления произв. занулённой матрицы "W" на вектор "y"
    ...
    def calculate_w(self):  # Функция расчёта матрицы W
        ...
        self.w = w  # Присвоение перменной w значения суммы матриц
    ...
    def create_y_vector(self, vector):  # Функции обработки вектора "y"
        ...
        self.y_vector.append(numpy_array)  # Добавить массив numpy в массив храниящий массивы "y"

    def calculate_zeroed_w_multiply_y(self):  # Функция вычисления произведения занулённой матрицы "W" на вектор "y"
        for numpy_array in self.y_vector:  # Для каждого вектора "y" (тестовго образа)
            ...
            tanh_zeroed_w_multiply_y = np.tanh(zeroed_w_multiply_y)  # Находим "tanh" для эл. в полученной матрице
            ...
            while True:  # Бесконечный цикл
                ...
                tanh_zeroed_w_multiply_y = np.tanh(zeroed_w_multiply_y)  # Обновляем матрицу tanh(y)
    ...
    def recognize(self):  # Функция определения найденного образа
        recognized_image = np.sign(self.tanh_w_multiply_y[-1])  # Инициализация распознанного обрааза
        ...
        for numpy_array in self.x_vectors:  # Для кажого вектора "x"
            ...
            for i in range(recognized_image.size):  # Для каждого элемента в распознанном образе
                ...
                if recognized_image[i] == numpy_array[i]:
                    ...
                    if temporary_counter_1 == recognized_image.size:  # Если все элементы равны
                ...
                if recognized_image[i] == negative_numpy_array[i]:
                    ...
                    if temporary_counter_2 == recognized_image.size:  # Если все элементы равны
                        self.recognized_image = - count  # Вернуть переменную
                        break  # Прекратить поиск

            count = count + 1

    # Функции отвечающие за вывод данных в еонсоль
    ...
    def print_zeroed_w(self):  # Вывести в консоль занулённую матрицу "W"
        print(self.zeroed_w)  # Вывести в консоль занулённую матрицу "W"
    ...
    def print_tanh_w_multiply_y(self):  # Вывести в консоль значения произведений занулённой матрицы "W" на вектор "y"
...
def main():  # Основная функция
    net = Hopfield(  # Создание экземляра "net" для проверки
        images=[[  # Образы
